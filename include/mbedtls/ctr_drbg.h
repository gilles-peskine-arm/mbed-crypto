...
/**
 * \brief               This function sets up a CTR_DRBG context and
 *                      performs the initial seeding.
 *
 ...
 *   the entropy length (default: #MBEDTLS_CTR_DRBG_ENTROPY_LEN bytes);
 * - the nonce, obtained by calling \p f_entropy for
 *   half the entropy length;
 * - the \p custom string.
 *
 ...
 *                      the entropy length or \p len is too large.
...
/**
 * \brief               This function sets up a CTR_DRBG context and
 *                      performs the initial seeding with an explicit nonce.
 *
 * This function is equivalent to mbedtls_ctr_drbg_seed(),
 * but takes an explicit nonce instead of obtaining it from
 * the entropy source.
 *
 * \note                The nonce value should not be repeated over the lifetime
 *                      of the device or application using CTR_DRBG.
 *                      See SP 800-90A §8.6.7 for more information about
 ...
 * \return              #MBEDTLS_ERR_CTR_DRBG_ENTROPY_SOURCE_FAILED on failure.
 */
int mbedtls_ctr_drbg_seed_with_nonce( mbedtls_ctr_drbg_context *ctx,
                   int (*f_entropy)(void *, unsigned char *, size_t),
                   void *p_entropy,
