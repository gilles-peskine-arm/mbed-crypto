...
KEY_TYPE_FROM_CURVE_TEMPLATE = b'''if (%(tester)s(type)) {
            append_with_curve(&buffer, buffer_size, &required_size,
                              "%(builder)s", %(builder_length)d,
                              PSA_KEY_TYPE_GET_CURVE(type));
        } else '''

KEY_TYPE_FROM_GROUP_TEMPLATE = b'''if (%(tester)s(type)) {
            append_with_group(&buffer, buffer_size, &required_size,
                              "%(builder)s", %(builder_length)d,
                              PSA_KEY_TYPE_GET_GROUP(type));
        } else '''

ALGORITHM_FROM_HASH_TEMPLATE = b'''if (%(tester)s(core_alg)) {
            append(&buffer, buffer_size, &required_size,
                   "%(builder)s(", %(builder_length)d + 1);
            append_with_alg(&buffer, buffer_size, &required_size,
...
class MacroCollector:
    ...
    _define_directive_re = re.compile(rb'\s*#\s*define\s+(\w+)' +
                                      rb'(?:\s+|\((\w+)\)\s*)' +
                                      rb'(.+)')
    _deprecated_definition_re = re.compile(rb'\s*MBEDTLS_DEPRECATED')
    ...
    def read_line(self, line):
        ...
        expansion = re.sub(rb'/\*.*?\*/|//.*', rb' ', expansion)
        if re.match(self._deprecated_definition_re, expansion):
            ...
            return
        if name.endswith(b'_FLAG') or name.endswith(b'MASK'):
            # Macro only to build actual values
            return
        elif (name.startswith(b'PSA_ERROR_') or name == b'PSA_SUCCESS') \
           and not parameter:
            self.statuses.add(name)
        elif name.startswith(b'PSA_KEY_TYPE_') and not parameter:
            self.key_types.add(name)
        elif name.startswith(b'PSA_KEY_TYPE_') and parameter == b'curve':
            self.key_types_from_curve[name] = name[:13] + b'IS_' + name[13:]
        elif name.startswith(b'PSA_KEY_TYPE_') and parameter == b'group':
            self.key_types_from_group[name] = name[:13] + b'IS_' + name[13:]
        elif name.startswith(b'PSA_ECC_CURVE_') and not parameter:
            self.ecc_curves.add(name)
        elif name.startswith(b'PSA_DH_GROUP_') and not parameter:
            self.dh_groups.add(name)
        elif name.startswith(b'PSA_ALG_') and not parameter:
            if name in [b'PSA_ALG_ECDSA_BASE',
                        b'PSA_ALG_RSA_PKCS1V15_SIGN_BASE']:
                # Ad hoc skipping of duplicate names for some numerical values
                return
            self.algorithms.add(name)
            # Ad hoc detection of hash algorithms
            if re.search(rb'0x010000[0-9A-Fa-f]{2}', expansion):
                self.hash_algorithms.add(name)
            # Ad hoc detection of key agreement algorithms
            if re.search(rb'0x30[0-9A-Fa-f]{2}0000', expansion):
                self.ka_algorithms.add(name)
        elif name.startswith(b'PSA_ALG_') and parameter == b'hash_alg':
            if name in [b'PSA_ALG_DSA', b'PSA_ALG_ECDSA']:
                # A naming irregularity
                tester = name[:8] + b'IS_RANDOMIZED_' + name[8:]
            else:
                tester = name[:8] + b'IS_' + name[8:]
            self.algorithms_from_hash[name] = tester
        elif name.startswith(b'PSA_KEY_USAGE_') and not parameter:
    ...
    def _make_return_case(name):
        return b'case %(name)s: return "%(name)s";' % {b'name': name}

    @staticmethod
    def _make_append_case(name):
        template = (b'case %(name)s: '
                    b'append(&buffer, buffer_size, &required_size, "%(name)s", %(length)d); '
                    b'break;')
        return template % {b'name': name, b'length': len(name)}

    @staticmethod
    def _make_bit_test(var, flag):
        return BIT_TEST_TEMPLATE % {b'var': var,
                                    b'flag': flag,
                                    b'length': len(flag)}
    ...
    def _make_key_type_from_curve_code(builder, tester):
        return KEY_TYPE_FROM_CURVE_TEMPLATE % {b'builder': builder,
                                               b'builder_length': len(builder),
                                               b'tester': tester}

    @staticmethod
    def _make_key_type_from_group_code(builder, tester):
        return KEY_TYPE_FROM_GROUP_TEMPLATE % {b'builder': builder,
                                               b'builder_length': len(builder),
                                               b'tester': tester}
    ...
    def _make_ecc_key_type_code(self):
        ...
        return b''.join([make(k, d[k]) for k in sorted(d.keys())])
    ...
    def _make_algorithm_from_hash_code(builder, tester):
        return ALGORITHM_FROM_HASH_TEMPLATE % {b'builder': builder,
                                               b'builder_length': len(builder),
                                               b'tester': tester}
    ...
    def _make_key_usage_code(self):
        return b'\n'.join([self._make_bit_test(b'usage', bit)
    ...
    def write_file(self, output_file):
        ...
        """
        data = {}
        data[b'status_cases'] = self._make_status_cases()
        data[b'ecc_curve_cases'] = self._make_ecc_curve_cases()
        data[b'dh_group_cases'] = self._make_dh_group_cases()
        data[b'key_type_cases'] = self._make_key_type_cases()
        data[b'key_type_code'] = (self._make_ecc_key_type_code() +
                                 self._make_dh_key_type_code())
        data[b'hash_algorithm_cases'] = self._make_hash_algorithm_cases()
        data[b'ka_algorithm_cases'] = self._make_ka_algorithm_cases()
        data[b'algorithm_cases'] = self._make_algorithm_cases()
        data[b'algorithm_code'] = self._make_algorithm_code()
        data[b'key_usage_code'] = self._make_key_usage_code()
